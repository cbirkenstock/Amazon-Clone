# Our link:
https://gitlab.oit.duke.edu/ccb65/mini-amazon-skeleton
...
# Update since last milestone

Eloise (Cart / Order)
Since the last milestone, I have implemented the full cart and order functionality. Users can add items to cart from the homepage (Shop) or from individual item pages. In their cart, users can edit the quantity, delete items from cart, and freely move items back and forth between cart and saved for later. Going to checkout will automatically move unavailable items to the user’s saved for later, and the user must input an address to be able to checkout. On checkout, it checks that everything is valid before placing the order.
Annie (Account / Purchases)
I have worked to redesign much of the website and added images for each user. I have made it so users can update their own information and polished the balance system, and also added order tables in the user’s profile so they can see their past orders with links to the order pages for each of those. I also made a search function for parsing orders with different ordering types. Sellers now have a public profile that users can visit.
Ali (Products)
I have implemented a search function to allow users to browse the products catalog and sort the results by multiple different orderings. I have split this into pages each showing 10 products. I have also made products have individual products pages displaying more information about them including the sellers, the quantity, the reviews, and an add to cart option. 
Gautam (Feedback / Messaging)
I made many improvements to my section since the last milestone. I wrote many new functions in my main reviews.py controller (paths to new pages, such as product and seller rating summary pages) as well my productreview.py and sellerreview.py models (new queries to create new tables to be displayed on my page). Now, users can search a sortable table of reviews, edit pre existing reviews, delete reviews and more, which are all new features that I added since the last milestone. On top of this, I added some data protection elements to my insert functions and made several aesthetic changes.
Colin (Inventory / Order Fulfillment)
I made many changes to the seller page. Now, you can see dynamic charts with analytics about your sales, and can also search for your own products that you are selling or orders you need to fulfill. I have also added tables to display this information. I made it so you can edit information about the products and delete them from the website, and also added buttons to fulfill orders.
